...
// variable 定义变量
    ...
    this.sDataTableWidth = this.sDataTable.css("width");
// clean 清除以前的数据
    ...
    $("#header_h").css("left", 0 + "px");
    $("#header_v").css("top", 0 + "px");
// 内容拷贝
    ...
    this.stableV.parent().css("width", this.sDataTableWidth);
    // 拷贝colgroup 到tablev
    ...
    // 位置设置
      ...
      this.stableT.parent().width(twidth/this.headerRows);
      $("#header_h").width(wrap.width() - tablet.width()-17);
      data.css('width', wrap.width() - tablet.width());
      data.css("height", wrap.height() - tablet.height()+17);
      this.sDataTable.css('width', wrap.width()-17);
      this.sDataTable.css("height", wrap.height());
      this.stableH.parent().width(wrap.width()-tablet.width()-17);
	...
	var wh =$("#tableT").height();
	  ...
	  this.sDataTable.css('margin-left', -wt);
